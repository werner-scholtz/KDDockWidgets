...
| --- | --- | --- | --- | --- | --- |
| Windows | Yes | Yes | Yes | Yes, native title bar | Closest current match to feature parity across the POC |
...
Windows is the current reference platform for the POC. macOS follows the same
broad architecture, but its richer native behavior should still be treated as a
first-pass runner path until it is re-smoke-tested on real hardware.

Wayland intentionally stays on deferred detach for new tear-offs and uses a
fallback whole-window docking gesture instead of title-bar parity behavior.
X11 is currently an upstream Flutter/Linux multi-window blocker rather than a
useful signal about KDDockWidgets docking behavior.
